...
const CampaignWizard = ({ onClose, currentUser }) => {
  ...
  // UPDATED: All mockups now have left-aligned text content
  const FacebookMockup = ({ content, image }) => (
    <div className="bg-white h-full">
      ...
      <div className="p-3 border-b border-gray-200 text-left">
        ...
        {content.headline && <p className="font-medium mb-2 text-sm whitespace-pre-line text-left">{content.headline}</p>}
        {content.description && <p className="text-sm text-gray-700 mb-3 whitespace-pre-line text-left">{content.description}</p>}
        {image && <img src={image} alt="Ad" className="w-full rounded-lg mb-3" style={{ maxHeight: '200px', objectFit: 'cover' }} />}
        {content.callToAction && <button className="bg-blue-600 text-white px-4 py-2 rounded text-sm font-medium w-full text-center">{content.callToAction}</button>}
      </div>
    </div>
  )
    ...
    <div className="bg-white h-full">
      ...
      <div>
        <div className="flex items-center space-x-2 p-3">
          ...
          <div className="flex-1 text-left"><p className="font-semibold text-sm">ihr_unternehmen</p><p className="text-xs text-gray-500">Gesponsert</p></div>
        ...
        <div className="p-3 text-left">
          ...
          {content.headline && <p className="text-sm text-left"><span className="font-semibold">ihr_unternehmen</span> {content.headline}</p>}
          {content.description && <p className="text-sm text-gray-700 mt-1 whitespace-pre-line text-left">{content.description}</p>}
    ...
    <div className="bg-black h-full relative text-white">
      ...
      <div className="relative h-full flex items-center justify-center">
        ...
        <div className="absolute bottom-20 left-4 right-16 text-left">
          ...
          {content.headline && <p className="font-medium mb-1 text-sm whitespace-pre-line text-left">{content.headline}</p>}
          {content.description && <p className="text-sm opacity-90 whitespace-pre-line text-left">{content.description}</p>}
  ...
  const GoogleAdsMockup = ({ content, image }) => (
    <div className="bg-white h-full">
      ...
      <div className="p-4 text-left">
        ...
        {content.headline && <h3 className="text-blue-600 text-lg font-medium mb-1 underline text-left">{content.headline}</h3>}
        <p className="text-green-700 text-sm mb-2 text-left">www.ihr-unternehmen.de</p>
        {content.description && <p className="text-gray-700 text-sm leading-relaxed mb-3 whitespace-pre-line text-left">{content.description}</p>}
  ...
  const SpotifyMockup = ({ content, image }) => (
    <div className="bg-black h-full text-white">
      <div className="bg-green-500 p-3 flex items-center space-x-2">
        <div className="w-8 h-8 bg-black rounded-full flex items-center justify-center"><Music className="w-4 h-4 text-white" /></div>
        <span className="font-semibold">Spotify</span>
      </div>
      <div className="p-4 text-left">
        <div className="mb-4">
          <p className="text-xs text-gray-400">GESPONSERT</p>
          <h3 className="text-lg font-bold mt-1 text-left">{content.headline || 'Entdecken Sie jetzt'}</h3>
        </div>
        <div className="flex items-center space-x-3 mb-4">
          {image ? (
            <img src={image} alt="Ad" className="w-16 h-16 rounded" />
          ) : (
            <div className="w-16 h-16 bg-gradient-to-r from-purple-500 to-pink-500 rounded flex items-center justify-center">
              <Music className="w-8 h-8 text-white" />
            </div>
          )}
          <div className="text-left">
            <p className="font-medium text-sm">{content.headline || 'Ihr Angebot'}</p>
            <p className="text-xs text-gray-400">Ihr Unternehmen</p>
          </div>
        </div>
        {content.description && <p className="text-sm text-gray-300 mb-4 whitespace-pre-line text-left">{content.description}</p>}
        {content.callToAction && <button className="bg-green-500 text-white px-4 py-2 rounded-full text-sm font-medium">{content.callToAction}</button>}
      </div>
    </div>
  )
  ...
  const SnapchatMockup = ({ content, image }) => (
    <div className="bg-white h-full">
      <div className="bg-yellow-400 p-3 flex items-center space-x-2">
      ...
      <div className="p-4 text-left">
        <div className="mb-3">
          <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full">Gesponsert</span>
        </div>
        {content.headline && <h3 className="font-bold mb-2 text-left">{content.headline}</h3>}
        {content.description && <p className="text-sm text-gray-700 mb-3 whitespace-pre-line text-left">{content.description}</p>}
        {image && <img src={image} alt="Ad" className="w-full rounded-lg mb-3" style={{ maxHeight: '200px', objectFit: 'cover' }} />}
        {content.callToAction && <button className="bg-yellow-400 text-black px-4 py-2 rounded-full text-sm font-medium w-full">{content.callToAction}</button>}
      </div>
    </div>
  )
    ...
    <div className="bg-white h-full">
      ...
      <div className="p-4 text-left">
        <div className="flex items-center space-x-2 mb-3">
          ...
          <div className="text-left">
            <p className="font-semibold text-sm">u/ihr_unternehmen</p>
            <p className="text-xs text-gray-500">Gesponsert</p>
          </div>
        </div>
        {content.headline && <h3 className="font-medium mb-2 text-left">{content.headline}</h3>}
        {content.description && <p className="text-sm text-gray-700 mb-3 whitespace-pre-line text-left">{content.description}</p>}
        {image && <img src={image} alt="Ad" className="w-full rounded mb-3" style={{ maxHeight: '200px', objectFit: 'cover' }} />}
